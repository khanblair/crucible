...
Crucible runs a full optimization every weekday — a deliberate choice to keep the system visibly, continuously working toward its performance target rather than reporting once a week. This is a real trade-off, not a free upgrade: every accept/reject decision is a hypothesis test, and testing roughly 250 times a year against overlapping data makes it more likely that some parameter set eventually passes by luck alone rather than genuine improvement. Crucible's answer to that risk is not a slower schedule but a stricter gate underneath the daily attempts: the Evaluator's out-of-sample checks, the 10%/20% improvement hurdles, the change cooldown, and the Champion Zero comparison all still apply to every single run, so daily *attempts* are cheap but an actual *accepted change* still has to clear the same bar it always did. Attempt often, change rarely — that's what keeps daily cadence honest.
...
**Runtime and budget ceiling:** The optimization workflow has a strict 60-minute maximum runtime, and the daily monitoring workflow a 10-minute maximum. Both now run every weekday. On a public repository, GitHub Actions minutes on standard runners are unmetered, so the daily cadence carries no cost ceiling — the runtime limits exist to bound a single run's duration and catch a hung job, not to conserve a minutes budget. If any workflow exceeds its limit, GitHub terminates it and the Discord notification includes an alert that the run did not complete normally.
