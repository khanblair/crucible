...
An optimizer pointed at a strategy with no edge does not create an edge — it simply finds the least-losing configuration in-sample, with impressive-looking rigor. Phase 0 exists to make that visible, honestly, before anything else happens.

The base strategy with its initial hand-set parameters is run through a single, full multi-year backtest under the realistic execution model described below (spread, slippage, swap, intrabar ordering). The backtest reports, at minimum:
...
The pass criteria are simple: positive expectancy after costs across the full period, a profit factor above 1.15, and no single year contributing more than half of total profit. This result, whatever it is, gets committed to the repository as `docs/phase0_report.md` so the foundation of the whole system is on record — and it's shown on the dashboard for exactly the same reason.

**Phase 0 does not gate the optimization or evolution loops.** An earlier version of this design treated a Phase 0 pass as a hard prerequisite before the loop could run at all. That has been deliberately relaxed: the loop now runs every 8 hours regardless of Phase 0's verdict, because the fastest way to find a strategy that does have an edge is to keep searching, not to wait. What Phase 0 still guarantees is honesty, not permission — a FAIL verdict does not stop the system, but it also never gets hidden. Every report says plainly whether the base strategy has proven an edge, and every accepted change still has to independently prove itself against the same out-of-sample Evaluator gates described below, Phase 0 result notwithstanding.
...
**Genomes, slots, and modules:** A *genome* is a full strategy structure — a specific choice for each *slot*, where a slot is a stage of the strategy with more than one human-written implementation to choose from. The two slots are the entry signal (today's EMA-pullback logic, or alternatives — a breakout entry, a mean-reversion entry, or a volatility-squeeze breakout that waits for a period of unusually tight volatility and trades the range break that follows) and the exit style (today's ATR-trail-with-half-profit-lock logic, a simpler fixed-multiple exit, or a time-stop exit that forces a close after a fixed number of bars if neither the stop nor the target has resolved). The session filter and the candle-quality filter are shared by every genome and never vary; the 1-hour trend confirmation and the RSI filter are used by the entry modules for which they're a meaningful part of the hypothesis, not forced onto every one — volatility-squeeze breakout and mean-reversion both intentionally omit at least one, since testing a genuinely different signal source (not a variation on the RSI/EMA-trend family already shown to lack edge) is the entire point of adding them. Each module is small, human-authored, and tested in isolation exactly like every other piece of Crucible's logic; no module is ever written by an AI model.

**When it runs:** Genome evolution is not on the daily optimization schedule. It triggers only when two things are both true: the last five optimization runs have each shown the active strategy losing money out-of-sample, and the parameter search itself has stopped finding meaningful improvement — the signature of being stuck with broken logic, not merely unlucky. When triggered, it screens every genome combination with a quick backtest, takes the most promising few, and runs the full Optuna parameter search on each of those — the same rigor applied to numbers today is applied to structures here. A winning candidate must then pass every existing Evaluator gate, plus a stricter bar reserved for structural change: a larger margin of improvement, and it must beat Champion Zero outright, not merely the current baseline. Changing strategy logic is a bigger commitment than tuning a number, and it is held to a correspondingly higher standard. If nothing clears that bar, the system reports so, keeps the current genome, and does not try again for a full quarter.
...
Crucible runs a full optimization every 8 hours, aligned to 00:00 / 08:00 / 16:00 East Africa Time (EAT, UTC+3 fixed, no daylight saving) — a deliberate choice to keep the system visibly, continuously working toward its performance target rather than reporting once a day or once a week. This is a real trade-off, not a free upgrade: every accept/reject decision is a hypothesis test, and testing roughly a thousand times a year against overlapping data makes it more likely that some parameter set eventually passes by luck alone rather than genuine improvement. Crucible's answer to that risk is not a slower schedule but a stricter gate underneath the frequent attempts: the Evaluator's out-of-sample checks, the 10%/20% improvement hurdles, the change cooldown, and the Champion Zero comparison all still apply to every single run, so frequent *attempts* are cheap but an actual *accepted change* still has to clear the same bar it always did. Attempt often, change rarely — that's what keeps a fast cadence honest.

**Monitoring run (every 8 hours, lightweight, under 10 minutes):** Pulls the latest candles, appends to the paper-forward signal log, updates the performance log and dashboard, checks for performance decay, and posts a brief Discord status. It also runs the deterministic regime classification: if the prevailing market regime has changed, the monitor swaps the active pointer to the matching pre-validated parameter set the same run. Switching among pre-validated sets is routing, not optimization — it involves no new hypothesis test — so it happens outside the cooldown, and every swap is reported to Discord. No optimization happens in this run. No weekday restriction — forex is closed on weekends anyway, and the data-refresh logic already treats a quiet weekend as a normal no-op, never a failure.

**Optimization run (every 8 hours, or immediately when a decay trigger fires):** The full five-move loop described below, including Optuna search and Evaluator gating. The decay trigger fires when the active parameters' rolling 20-trade expectancy drops below zero or drawdown crosses the 8% monitoring alert level — since optimization already runs every 8 hours, a decay trigger simply means the next scheduled run is treated as urgent rather than waiting for anything. Note that this 8% alert is deliberately tighter than the Evaluator's 12% rejection ceiling: the monitor is meant to react before the situation is bad enough to fail the gate.

**Cooldown:** After any accepted parameter change, no further changes may be accepted for five trading days, regardless of what the optimizer finds. This prevents churn and gives every change enough forward data to be judged — it's the main safeguard that keeps frequent attempts from becoming frequent rewrites.

**Phase 0 is informational, not a gate:** the one-time proof of a base edge (below) is generated once and shown on the dashboard, but it no longer blocks the optimization or evolution loops from running. This is a deliberate choice to prioritize continuous refinement over waiting for a hard proof-of-edge milestone — every candidate, Phase-0-proven strategy or not, still has to clear the Evaluator's out-of-sample gates to be accepted. Nothing here promises the underlying strategy has an edge; it promises that if a genuinely better version of it exists within reach of the search, the system won't stop looking for it, and won't accept anything that can't prove itself on data it has never seen.

**All human-facing timestamps display in EAT.** Discord messages, the dashboard's "Generated" time, and every scheduled-run reference show East Africa Time. Every internal calculation — walk-forward windows, the cooldown's trading-day count, regime classification — still runs on UTC underneath; only the display layer converts, so there is no risk of a display-only timezone choice silently shifting an actual decision boundary.
...
The data layer is the foundation of the entire system. Crucible uses two data sources with a firm rule about which one is primary — and, since Dukascopy's free feed has shown itself capable of silently stalling, an honest, always-visible fallback rather than a system that quietly keeps re-analyzing stale data as if it were current.

**Decision data — Dukascopy (primary):** All backtesting, all optimization, all Evaluator comparisons, and the paper-forward signal log are built primarily on Dukascopy data. Dukascopy provides free tick-level historical EUR/USD data going back several years with no account registration, downloaded from https://www.dukascopy.com/swiss/english/marketwatch/historical/, cleaned, and resampled into 15-minute and 1-hour bars — with the tick history distilled offline into the compact intrabar-ordering table described in the execution realism section. Dukascopy publishes with roughly a one-day delay, which is fully compatible with a system that evaluates at and after market close. The dataset is refreshed every optimization run.

**Dukascopy is a free public feed, and it can fail — every refresh tells the truth about whether it did.** Every fetch attempt is classified into one of three outcomes, never conflated: `ok` (real data), `empty` (HTTP 200, no ticks — a genuine market holiday, entirely routine), or `rejected`/`unreachable` (a non-200 response or a network failure — the feed pushing back, which is *not* the same thing as a holiday and must never be silently treated as one). A refresh only reports success when the committed data genuinely advanced to a later date than before; re-downloading an already-covered day and getting the same content back is reported as exactly what it is — no progress — never as a false "refreshed through X."

**Decision-data fallback — Twelve Data:** When Dukascopy is confirmed struggling (at least one rejected or unreachable day, not just a quiet holiday stretch) and a genuine trading day is still missing afterward, Crucible bridges that specific gap with Twelve Data — real market data from a different vendor, used only to fill the confirmed hole, never silently. Every fallback-sourced bar is logged in the refresh summary and called out explicitly in the Discord report and on the dashboard (a visible "backup data source was used" note), because a different vendor's aggregation methodology is a real, if usually small, difference worth being honest about — which is exactly what the monthly reconciliation job (below) already exists to measure.

**Context data — Twelve Data API:** Independent of the fallback role above, the monitoring run also pulls the most recent EUR/USD candles from the Twelve Data REST API to give same-run context for the dashboard and Discord status. Twelve Data requires only an email address to register — no broker account, no geographic restrictions, no credit card — and its free tier of 800 requests per day far exceeds Crucible's needs. The API key is stored as a GitHub Actions secret under `TWELVE_DATA_KEY` and never appears in code.

**Data freshness is reported, never assumed.** Every monitoring and optimization report carries an explicit "trading days stale" count computed by comparing the committed data's last date against what should be available given Dukascopy's usual one-day publish delay. When that count is greater than zero, a visible warning appears in Discord and a prominent banner appears at the top of the dashboard — so a stalled feed is something the system says out loud, not something a human has to notice by comparing today's numbers against yesterday's and finding them suspiciously identical.

**Feed reconciliation:** Because the two feeds aggregate prices differently, a monthly reconciliation job compares them over their overlap and logs the mean absolute difference. If divergence exceeds 0.5 pips on average, an alert is raised in Discord — this catches silent data-quality drift before it can distort the performance picture, and matters more now that Twelve Data can occasionally sit directly in the decision-data path as a fallback, not just as context.

No synthetic data is used anywhere. No random number generators are used to simulate price. Every number the system works with comes from real market prices — from one of exactly two named, real vendors, and the report always says which.
...
**Rolling, never-reused test windows:** The historical dataset is split into training and out-of-sample test chunks. The optimizer only ever sees the training chunks. Critically, the out-of-sample window rolls forward with each optimization run — each daily run is judged primarily on market data that did not exist at the time of the previous run. No test window is reused across runs for accept/reject decisions.
...
**Final untouched holdout:** The most recent six months of history are permanently quarantined. Neither Optuna nor the daily Evaluator ever touches this data. Once per quarter, an audit run evaluates the full parameter-change history against the holdout. The audit has consequences: if it shows the evolved system underperforming Champion Zero on the quarantined data, acceptance of new parameter changes is paused automatically until a human engineer reviews the audit and re-enables the loop through a pull request. An examiner whose verdict carries no consequence is not an examiner.

**Improvement margin and trade floor:** As specified in the Verification move, a candidate must beat the baseline by a real margin with a real sample size. This is the practical defense against the multiple-testing problem now that optimization runs daily rather than weekly (roughly 250 attempts a year, not 52): a bare ">" would let a lucky-but-worthless configuration through eventually; a 10% relative-improvement hurdle plus the 30-trade floor makes that rare instead of routine. The trade-off is real and deliberate — daily attempts in exchange for a stricter bar per attempt, not a free upgrade to speed.
...
When the regime changes, the daily monitor swaps `active.json` to the matching pre-validated set the same day, independent of the optimization run. This is routing, not optimization — no new hypothesis is tested — so it lives outside the cooldown, and every swap is announced in Discord.

**Staleness rule:** a regime parameter set that has not been re-validated within 90 days is flagged stale on the dashboard. When the monitor switches into a stale regime, two things happen: the next optimization run targets that regime's set first, regardless of which regime prevails that day, and until that revalidation completes, Champion Zero's parameters are used as the safe fallback for that regime. Stale, unexamined parameters are treated as untrusted.

When the daily optimization run executes normally, it optimizes the parameter set belonging to the currently prevailing regime, tested on historical windows filtered to that same regime.
...
**Runtime and budget ceiling:** The optimization workflow has a strict 60-minute maximum runtime, and the monitoring workflow a 10-minute maximum. Both now run every 8 hours, every day of the week (no weekday restriction — a weekend run simply finds nothing new and says so). On a public repository, GitHub Actions minutes on standard runners are unmetered, so this cadence carries no cost ceiling — the runtime limits exist to bound a single run's duration and catch a hung job, not to conserve a minutes budget. If any workflow exceeds its limit, GitHub terminates it and the Discord notification includes an alert that the run did not complete normally.
...
**GitHub Pages Dashboard:** A live webpage hosted from the repository shows the current active regime and parameters, staleness flags for any regime set not validated within 90 days, a market-data freshness banner (prominent, amber, impossible to miss whenever the feed is behind schedule), the equity curve over time, the Champion Zero vs. evolved-system comparison curves, the live-vs-backtest divergence metric, the win rate trend, the drawdown history, and a log of every parameter change ever made with dates and reasons. This updates automatically after every run, and its "Generated" timestamp — like every other timestamp anywhere in the system's reporting — displays in East Africa Time.
...
**Discord Channel:** A dedicated channel receives a structured embed after every run — a brief status from monitoring runs (including any regime swaps), and a full report from optimization runs including the outcome (accepted or rejected), the specific net-of-cost metrics before and after, which parameters changed and by how much, the champion-vs-challenger standing, and any warnings or anomalies. A data-freshness field appears on any report where the market data is behind schedule, naming exactly how many trading days stale it is and whether a fallback source had to be used.

**Plain-English section:** Every Discord report and the dashboard also carry a non-technical summary generated by deterministic code (never by the language model — money figures must come from arithmetic). It translates pips into dollars at a configurable small position size, states what the recent practice signals would have made or lost, projects the pace per day, week and year with an explicit "if the market kept behaving the same way" caveat, and explains parameter decisions as "found better settings and switched" or "tried new settings but kept the old ones." Simulated results are always labeled as such. Since the goal is a specific daily target, the same summary states the gap plainly — how far short of (or past) the configured `daily_target_usd` the recent pace actually is — so the target is a visible measurement every day, not a claim.
...
Crucible does not guarantee profitability, and it does not guarantee a minimum daily return, even a small one. Every report measures forward performance against a configured daily target (`reporting.daily_target_usd` in `config/settings.json`) so progress toward it is visible every day, but that target is a goal the system searches toward, never a floor it can promise. Better parameters in backtesting improve the probability of better forward performance, but past performance on historical data does not guarantee future results — no automated system, this one included, can promise a minimum outcome regardless of market conditions. The system reduces the risk of running a degraded strategy by keeping parameters current, but market conditions can change in ways historical data did not anticipate.
